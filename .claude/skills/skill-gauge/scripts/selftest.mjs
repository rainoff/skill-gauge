...
// 外掛揭露（plugin disclosure，1.2.1＋R1 修正；spec: gauge/plugin-disclosure-dev/spec.md）：偵測＋揭露句，陽性陰性成對
{
  ...
  t('外掛偵測：manifest 壞 JSON 不炸、manifest=null、payload 檔案 marker 仍命中；預設 skills/ 不靠 manifest 仍判 scope=true', bh?.pluginRoot === broot && bh.manifest === null && bh.hasHooks === true && bh.skillsScope === true);
  const bout = path.join(broot, 'other', 'z2'); fs.mkdirSync(bout, { recursive: true }); fs.writeFileSync(path.join(bout, 'SKILL.md'), 'z2');
  t('外掛偵測：壞 JSON＋不在預設 skills/ 下→自訂條目未知，scope=null', detectPluginContext(bout)?.skillsScope === null);
  ...
  t('外掛偵測（R1-3）：manifest 的 hooks／mcpServers 為 null 也算有 key（無 payload 檔）；預設 skills/ → scope=true', nh?.hasHooks === true && nh?.hasMcp === true && nh?.skillsScope === true);
  ...
  // R2：官方 skills 語意（string 形式、'.'＝root 單 skill、自訂容器、跳脫條目防偽造、預設永遠掃）
  const dot = path.join(pd, 'dot-root'); fs.mkdirSync(path.join(dot, '.claude-plugin'), { recursive: true });
  ...
  fs.writeFileSync(path.join(csk, 'SKILL.md'), 's');
  t('外掛偵測（R2）：skills 為 string "./custom/" 容器→其下 skill scope=true', detectPluginContext(csk)?.skillsScope === true);
  const esc = path.join(pd, 'esc'), esk = path.join(pd, 'esc', 'x');
  fs.mkdirSync(path.join(esc, '.claude-plugin'), { recursive: true }); fs.mkdirSync(esk, { recursive: true });
  fs.writeFileSync(path.join(esc, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'escy', skills: ['./..', '../foo', '/abs', 'x'] }));
  fs.writeFileSync(path.join(esk, 'SKILL.md'), 'x');
  t('外掛偵測（R2）：跳脫／絕對／無 ./ 前綴的條目全被忽略→scope=false，不被 ".." 偽造成範圍內', detectPluginContext(esk)?.skillsScope === false);
  const flood = path.join(pd, 'flood'); fs.mkdirSync(flood, { recursive: true });
  for (let i = 0; i < 201; i++) fs.writeFileSync(path.join(flood, `f${String(i).padStart(3, '0')}.md`), 'no refs here');
  const fh = detectPluginContext(flood);
  t('外掛偵測（R2-5）：201 個 .md 觸頂→回 inert 物件記 scanTruncated，不回 null 靜默', fh !== null && fh.pluginRoot === null && fh.scanTruncated === true && fh.mcpRefs.length === 0);
  t('外掛偵測（R2-5）：inert 物件的揭露句為空字串（不印任何字）', pluginBoundaryNote({ subjectPlugin: fh }) === '');
  const lroot = path.join(pd, 'links'); fs.mkdirSync(lroot, { recursive: true });
  const ltarget = path.join(pd, 'link-target'); fs.mkdirSync(ltarget, { recursive: true });
  fs.writeFileSync(path.join(ltarget, 'hidden.md'), '有 mcp__hidden__x');
  fs.writeFileSync(path.join(lroot, 'SKILL.md'), '本體乾淨');
  fs.symlinkSync(ltarget, path.join(lroot, 'sub'));
  t('外掛偵測（R2）：目錄 symlink 不跟隨，連結後的 refs 不出現', JSON.stringify(detectPluginContext(lroot)?.mcpRefs ?? []) === '[]' || detectPluginContext(lroot) === null);
  const cjk = path.join(pd, 'cjk'); fs.mkdirSync(cjk, { recursive: true });
  fs.writeFileSync(path.join(cjk, 'SKILL.md'), '叫mcp__cjk__x再叫mcp__b__2與mcp__a__1');
  const ch = detectPluginContext(cjk);
  t('外掛偵測（R2）：\\b 在 CJK 緊鄰處有效、refs 輸出排序', JSON.stringify(ch?.mcpRefs) === '["mcp__a__1","mcp__b__2","mcp__cjk__x"]');
  ...
  const bSoft = decisionFirstLines(mkPlugR({ pluginRoot: '/p', manifest: { name: 'soft-plug', version: null }, hasHooks: false, hasMcp: false, skillsScope: null, mcpRefs: [] })).find((l) => l.startsWith('【邊界】')) || '';
  const bRefs = decisionFirstLines(mkPlugR(rh)).find((l) => l.startsWith('【邊界】')) || '';
  t('邊界行整合（R2）：軟外掛句與 refs-only 句都進得了邊界行', /位於外掛 soft-plug 的目錄樹下/.test(bSoft) && /mcp__jira__search/.test(bRefs) && /沙箱不掛任何 MCP/.test(bRefs));
