...
引擎出報告時已同時產 `page.html`——只有結論／情境地圖／發現／邊界／下一步，白話、手機可讀、亮暗色都行、**不含檢查項 id 與工程欄位**。要轉給不是執行者的人（主管、同事、skill 的擁有者），交這一頁，不要貼 report.md 的工程段。四條紀律它已內建：頭條與但書、修正值同一張卡（截走頭條也帶著界線）；每個派生數字旁有算式；絕對詞帶樣本量（「這 12 次裡沒有」不寫「零誤觸發」）；每條建議標「誰能動手」。環境裡有可視化 skill（viz-explain、artifact-design）可以再做互動版——選配，不取代這一頁。舊版報告（1.2.x 以前）沒有這一頁的資料，重出會得到誠實的缺頁說明，不會拿舊資料硬湊。
