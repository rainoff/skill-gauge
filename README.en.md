...
4. **A page for humans comes out automatically (`page.html`):** only the conclusion, a scenario map, findings, limits, and next steps — plain language, phone-readable. The headline sits on the same card as its caveats and correction range (screenshot the top and the limits travel with it); every derived number carries its formula; absolute statements carry their sample size; every suggestion names who can act on it. The full engineering version (concrete success/failure counts and their gap, how many judgments would flip it, which checks discriminate nothing, cost, every output (long ones truncated) and grading evidence) remains in `report.md` and `report.html`; the web version adds a browse-every-output section.
