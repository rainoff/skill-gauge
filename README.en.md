...
4. **A page for humans comes out automatically (`page.html`):** only the conclusion, a scenario map, findings, limits, and next steps — plain language, phone-readable. The headline sits on the same card as its caveats (screenshot the top and the limits travel with it); gate-check failures get a counterfactual bound (a worst-case envelope, not a bias correction); every derived number carries its formula or denominator; absolute statements carry their sample size; every suggestion names who can act on it; a stopped or data-poor run never fakes a comparison from partial data. The full engineering version (concrete success/failure counts and their gap, how many judgments would flip it, which checks discriminate nothing, cost, every output (long ones truncated) and grading evidence) remains in `report.md` and `report.html`; the web version adds a browse-every-output section.
...
- A skill that is part of a plugin, or that depends on hooks / MCP tools: the isolated environment copies only the skill folder — the plugin's hooks and MCP config do not come along, so both groups lack them. If the skill's effect depends on them, the measurement underestimates it or shows no difference at all. The engine auto-detects three file-level traces (a plugin manifest in an ancestor directory, the standard hook / MCP config files at the plugin root, and literal `mcp__` references in the skill's text) and writes what it found into the report's boundary line; when hook / MCP context is detected, a stop verdict carries a "possibly an underestimate" caveat. Dependencies not written into files are not detected — those must be added to the limits section by hand. Actually measuring plugin effects needs a separate design (see [roadmap](docs/roadmap.md)).
