...
3. **它先報成本，你點頭才執行。** 引擎開跑前先做已知答案檢查：拿你的一條全域規則、受測 skill 的名字各問一次，關門前答得出、關門後要答不出——這裡的「沙盒環境」是主目錄之外的空資料夾加三個啟動開關，隔的是你的規則、記憶、工具，不是作業系統層的安全沙箱（MCP 沒設檢查題；規則題判不出時報告會印 INCONCLUSIVE，看到就別當門關了）。然後**先執行不帶 skill 那一組**，每條檢查每次都過就停（這組題測不出 skill 的貢獻：模型本來就會，或題目太鬆）；沒全過才執行帶 skill 那組——壓力題與不該觸發的題不受停的影響，帶 skill 那組照樣執行當安全探針（停的是「幫不上忙」，不代表不會幫倒忙）。每份產出交給另一個新對話盲評；評分模型開跑前先拿一定該過、一定不該過的各一份考過（同模型、同提示、另一個新對話）。
...
**Windows：** AI 在第 3 步執行引擎時要帶 `--root D:\sg`（SKILL.md 第 4 步已寫，看到指令時確認一下；系統暫存目錄在使用者目錄底下，引擎會拒跑，原因見[細節版](docs/how-it-runs.md)）。**要多久（維護者 08-18 在 mac 實跑的時間，run 產物不進版控，只當估算）：** 測 clarify（5 題 × 3 次 × 4 組＋觸發題 30 次）約 54 分鐘；先 `--runs 1` 看看，教具（下面）三組約 9 分鐘。前提：`claude --version` 有回應、Node ≥ 18。
