...
- CLI 未知旗標靜默吞（critic 二輪實測：`all --model gpt-x` 被忽略照跑）：parseArgs 收下未知旗標而無人讀——加白名單拒絕或至少警告；matrix 格 re-report（`report --out <cell>`）的覆寫旗標豁免依賴 `__matrixCell`，獨立重出格子的路徑仍待修（既有「report --out 相對路徑」待修項同區）。
